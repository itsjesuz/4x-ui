...
export function useNodeMutations() {
  ...
  const syncMut = useMutation({
    mutationFn: (id: number) =>
      HttpUtil.post(`/panel/api/nodes/sync/${id}`),
    onSuccess: (msg) => { if (msg?.success) invalidate(); },
  });

  const syncAllMut = useMutation({
    mutationFn: () =>
      HttpUtil.post('/panel/api/nodes/syncAll'),
    onSuccess: (msg) => { if (msg?.success) invalidate(); },
  });
  ...
  return {
    ...
    sync: (id: number) => syncMut.mutateAsync(id),
    syncAll: () => syncAllMut.mutateAsync(),
    fetchUnsyncedClients: (id: number): Promise<Msg<{ toAdd: string[]; toDelete: string[]; dirty: boolean }>> =>
      HttpUtil.get(`/panel/api/nodes/unsynced/${id}`),
