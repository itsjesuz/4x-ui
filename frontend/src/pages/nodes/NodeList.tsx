...
interface NodeListProps {
  ...
  onSyncNode: (node: NodeRecord) => void;
  onSyncAll: () => void;
}
...
export default function NodeList({
  ...
  const columns = useMemo<ColumnsType<NodeRow>>(() => [
    {
      ...
      render: (_value, record) => record.transitive ? (
        ...
        <Space>
          ...
          {record.enable && record.status === 'online' && (
            <Tooltip title={t('pages.nodes.syncNode') || 'Sync Node'}>
              <Button type="text" size="small" icon={<SyncOutlined />} onClick={() => onSyncNode(record)} />
            </Tooltip>
          )}
    ...
    {
      title: t('pages.nodes.syncState') || 'Sync State',
      dataIndex: 'configDirty',
      align: 'center',
      render: (_value, record) => {
        if (record.transitive) return <span style={{ opacity: 0.4 }}>—</span>;
        if (!record.enable) return <Tag>{t('disabled')}</Tag>;
        if (record.status !== 'online') return <Tag color="default">{t('pages.nodes.statusValues.unknown')}</Tag>;

        return record.configDirty ? (
          <Tooltip title={t('pages.nodes.syncPendingTooltip') || 'Click to view unsynced changes and synchronize node.'}>
            <Tag color="warning" style={{ cursor: 'pointer' }} onClick={() => onSyncNode(record)}>
              {t('pages.nodes.outOfSync') || 'Out of Sync'}
            </Tag>
          </Tooltip>
        ) : (
          <Tag color="success">
            {t('pages.nodes.synced') || 'Synced'}
          </Tag>
        );
      },
    },
  ...
  ], [t, showAddress, relativeTime, latestVersion, onToggleEnable, onProbe, onEdit, onDelete, onUpdateNode, onSyncNode, nameByGuid]);
    ...
    <Card size="small" hoverable>
      <div className="toolbar">
        ...
        <Button icon={<SyncOutlined />} onClick={onSyncAll}>
          {t('pages.nodes.syncAll') || 'Sync All Nodes'}
        </Button>
                ...
                <div key={record.id} className="node-card">
                  <div className="card-head" onClick={() => toggleExpanded(record.id)}>
                    ...
                    <span className="node-name">
                      {record.name}
                      {record.configDirty && record.status === 'online' && record.enable && (
                        <Tag color="warning" style={{ marginInlineStart: 8, cursor: 'pointer' }} onClick={() => onSyncNode(record)}>
                          {t('pages.nodes.outOfSync') || 'Out of Sync'}
                        </Tag>
                      )}
                    </span>
                      ...
                      <Dropdown
                        ...
                        menu={{
                          items: [
                            ...
                            {
                              key: 'sync',
                              disabled: !record.enable || record.status !== 'online',
                              label: <><SyncOutlined /> {t('pages.nodes.syncNode')}</>,
                              onClick: () => onSyncNode(record),
                            },
                            {
                              key: 'edit',
          ...
          >
            {statsNode && (
              <div className="card-stats">
                ...
                {!statsNode.transitive && statsNode.enable && statsNode.status === 'online' && (
                  <div className="stat-row">
                    <span className="stat-label">{t('pages.nodes.syncState') || 'Sync State'}</span>
                    {statsNode.configDirty ? (
                      <Tag color="warning" style={{ cursor: 'pointer' }} onClick={() => { setStatsNode(null); onSyncNode(statsNode); }}>
                        {t('pages.nodes.outOfSync') || 'Out of Sync'}
                      </Tag>
                    ) : (
                      <Tag color="success">{t('pages.nodes.synced') || 'Synced'}</Tag>
                    )}
                  </div>
                )}
                <div className="stat-row">
