...
export default function ClientsPage() {
  ...
  const [viewMode, setViewMode] = useState<'table' | 'grid' | 'compact'>(
    () => (localStorage.getItem('clientsViewMode') as 'table' | 'grid' | 'compact') || 'table'
  );
    ...
    },
    {
      ...
      render: (_v, record) => (
        <div className="email-cell">
          ...
          {(record.telegramUsername || record.orderNumber) && (
            <div style={{ marginTop: 4, display: 'flex', gap: 4, alignItems: 'center', flexWrap: 'wrap' }}>
              {record.telegramUsername && (
                <Tooltip title={`${record.telegramFirstName || ''} ${record.telegramLastName || ''}`.trim() || undefined}>
                  <Tag
                    color="processing"
                    icon={<SendOutlined />}
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      window.open(`https://t.me/${record.telegramUsername}`, '_blank');
                    }}
                  >
                    @{record.telegramUsername}
                  </Tag>
                      ...
                      <div className={isMobile ? 'filter-bar mobile' : 'filter-bar'}>
                        ...
                        {!isMobile && (
                          <Space.Compact size="middle">
                            <Tooltip title={t('pages.clients.tableView', 'Table View')}>
                              <Button
                                icon={<TableOutlined />}
                            ...
                            <Tooltip title={t('pages.clients.gridView', 'Grid View')}>
                              <Button
                            ...
                            <Tooltip title={t('pages.clients.compactView', 'Compact View')}>
                              <Button
                                icon={<BarsOutlined />}
                              ...
                              <>
                                <Row gutter={[16, 16]} style={{ marginTop: 8 }}>
                                  {sortedClients.map((row) => {
                                    ...
                                    const isSelected = selectedRowKeys.includes(row.email);
                                    return (
                                      <Col key={row.email} xs={24} sm={12} md={8} lg={6}>
                                        <Card
                                          size="small"
                                          className={`client-grid-card ${isSelected ? 'is-selected' : ''}`}
                                          actions={[
                                            <Tooltip title={t('pages.clients.qrCode')} key="qr">
                                              <Button type="text" size="small" icon={<QrcodeOutlined />} onClick={() => onShowQr(row)} />
                                            </Tooltip>,
                                            <Tooltip title={t('pages.clients.clientInfo')} key="info">
                                              <Button type="text" size="small" icon={<InfoCircleOutlined />} onClick={() => onShowInfo(row)} />
                                            </Tooltip>,
                                            <Tooltip title={t('pages.inbounds.resetTraffic')} key="reset">
                                              <Button type="text" size="small" icon={<RetweetOutlined />} onClick={() => onResetTraffic(row)} />
                                            </Tooltip>,
                                            ...
                                            <Tooltip title={t('delete')} key="delete">
                                              <Button type="text" size="small" danger icon={<DeleteOutlined />} onClick={() => onDelete(row)} />
                                            </Tooltip>
                                          ]}
                                          title={
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                              ...
                                              <span style={{ fontWeight: 600, fontSize: 13, textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap', flex: 1 }} title={row.email}>
                                                {row.email}
                                              </span>
                                              <Switch
                                                checked={!!row.enable}
                                          ...
                                          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, padding: '4px 6px' }}>
                                            {row.subId && (
                                              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12 }}>
                                                ...
                                                <span style={{ fontFamily: 'monospace', textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap', maxWidth: '75%' }} title={row.subId}>{row.subId}</span>
                                              </div>
                                            )}
                                            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12 }}>
                                              <span style={{ opacity: 0.5 }}>{t('pages.clients.traffic')}:</span>
                                              <span style={{ fontWeight: 500 }}>{trafficLabel(row)}</span>
                                            </div>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12 }}>
                                              <span style={{ opacity: 0.5 }}>{t('pages.clients.remaining')}:</span>
                                              <Tag color={remainingColor(row)} style={{ margin: 0, fontSize: 11, padding: '0 6px', lineHeight: '18px' }}>{remainingLabel(row)}</Tag>
                                            </div>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12 }}>
                                              <span style={{ opacity: 0.5 }}>{t('pages.clients.duration')}:</span>
                                              <Tooltip title={expiryLabel(row)}>
                                                <Tag color={expiryColor(row)} style={{ margin: 0, fontSize: 11, padding: '0 6px', lineHeight: '18px' }}>{row.expiryTime ? expiryRelative(row) : '∞'}</Tag>
                                              </Tooltip>
                                            </div>
                                            {row.inboundIds && row.inboundIds.length > 0 && (
                                              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 12 }}>
                                                <span style={{ opacity: 0.5 }}>{t('pages.clients.attachedInbounds')}:</span>
                                                <span style={{ display: 'inline-flex', flexWrap: 'wrap', gap: 2, justifyContent: 'flex-end', maxWidth: '70%' }}>
                                                  {row.inboundIds.slice(0, 2).map((id) => {
                                                    const ib = inboundsById[id];
                                                    const proto = (ib?.protocol || '').toLowerCase();
                                                    const color = INBOUND_PROTOCOL_COLORS[proto] ?? 'default';
                                                    return (
                                                      <Tag key={id} color={color} style={{ margin: 0, fontSize: 10, padding: '0 4px', lineHeight: '16px' }}>
                                                        {ib?.remark?.trim() || ib?.tag || ''}
                                                      </Tag>
                                                    );
                                                  })}
                                                  {row.inboundIds.length > 2 && (
                                            ...
                                            {(row.telegramUsername || row.orderNumber || row.group) && (
                                              <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginTop: 4 }}>
                                                {row.group && <Tag color="geekblue" style={{ fontSize: 10, margin: 0 }}>{row.group}</Tag>}
                                                {row.telegramUsername && (
                                                  <Tag
                                                    ...
                                                    onClick={(e) => {
                                                      e.stopPropagation();
                                                      window.open(`https://t.me/${row.telegramUsername}`, '_blank');
                                                    }}
                                                  >
                                                    @{row.telegramUsername}
                                                  </Tag>
                                                )}
                                                {row.orderNumber && <Tag color="purple" style={{ fontSize: 10, margin: 0 }}>{row.orderNumber}</Tag>}
                                              </div>
                                            )}
                                          </div>
                                        </Card>
                              ...
                              <>
                                <Row gutter={[12, 12]} style={{ marginTop: 8 }}>
                                  {sortedClients.map((row) => {
                                    ...
                                    const ratioColor = ratio >= 100 ? '#ff4d4f' : ratio >= 85 ? '#faad14' : '#52c41a';

                                    return (
                                      <Col key={row.email} xs={12} sm={8} md={6} lg={6} xl={4} xxl={3}>
                                        <div className={`client-grid-compact-card ${isSelected ? 'is-selected' : ''}`} onClick={() => toggleSelect(row.email, !isSelected)}>
                                          <div className="compact-card-header">
                                            <Badge status={bucketBadgeStatus(bucket)} />
                                            <span className="compact-email" title={row.email}>{row.email}</span>
                                          </div>
                                          
                                          <div className="compact-traffic-bar">
                                            <div className="compact-traffic-fill" style={{ width: `${total > 0 ? ratio : 100}%`, background: total > 0 ? ratioColor : 'var(--ant-color-primary)' }} />
                                          </div>
                                          
                                          <div className="compact-stats">
                                            <span style={{ fontWeight: 500 }}>{trafficLabel(row)}</span>
                                            <Tag color={expiryColor(row)} style={{ margin: 0, fontSize: 10, padding: '0 4px', lineHeight: '16px' }}>{row.expiryTime ? expiryRelative(row) : '∞'}</Tag>
                                          </div>

                                          <div className="compact-card-actions" onClick={(e) => e.stopPropagation()}>
                                            <Tooltip title={t('pages.clients.clientInfo')}>
                                              <InfoCircleOutlined onClick={() => onShowInfo(row)} />
                                            </Tooltip>
                                            <Tooltip title={t('edit')}>
                                            ...
                                            <Dropdown
                                              ...
                                              menu={{
                                                items: [
                                                  { key: 'qr', label: <><QrcodeOutlined /> {t('pages.clients.qrCode')}</>, onClick: () => onShowQr(row) },
                                                  { key: 'reset', label: <><RetweetOutlined /> {t('pages.inbounds.resetTraffic')}</>, onClick: () => onResetTraffic(row) },
                                                  { key: 'delete', danger: true, label: <><DeleteOutlined /> {t('delete')}</>, onClick: () => onDelete(row) },
                                                ]
                                              }}
                                            >
                                              <MoreOutlined />
