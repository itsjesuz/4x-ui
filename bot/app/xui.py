...
def _parse_usage(raw: Any) -> ClientUsage:
    ...
    up = _extract_int(raw, "up")
    down = _extract_int(raw, "down")

    if not up and not down:
        used = _extract_int(raw, "usedTraffic", "used_traffic")
        if used > 0:
            up = used
...
class XuiClient:
    ...
    async def list_inbounds(self) -> list[dict[str, Any]]:
        """Return available inbounds from the panel.
            ...
            if isinstance(obj, list):
                ...
                for item in obj:
                    ...
                    result.append({
                        ...
                        "remark": str(item.get("remark", item.get("tag", ""))),
                        "protocol": str(item.get("protocol", "")),
                    })
                return result
        except Exception as exc:
            log.warning("list_inbounds failed: %s", exc)
        return []
    ...
    async def find_client(self, email: str) -> dict[str, Any] | None:
        ...
        for client in await self.list_clients():
            if str(client.get("email", "")).lower() == email_lower:
                return client
        return None
