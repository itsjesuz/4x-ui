...
class EditLocationFlow(StatesGroup):
    ...
    waiting_token = State()
    picking_inbounds = State()   # interactive button picker (fetched from panel)
    waiting_inbounds = State()   # manual text fallback
    waiting_sub = State()
...
async def edit_loc_token(
    ...
    # Try fetching inbounds from the panel using the entered credentials
    await _show_inbound_picker(message, state, base_url, api_token, data["inbound_ids"], loc_id)


async def _show_inbound_picker(
    message: Message,
    state: FSMContext,
    base_url: str,
    api_token: str,
    ...
    loc_id: int,
    *,
    flow_state_picking=None,
    flow_state_manual=None,
) -> None:
    """Fetch inbounds from panel and show toggleable picker. Falls back to text prompt.

    flow_state_picking / flow_state_manual allow callers with different FSM groups
    (e.g. AddLocationFlow vs EditLocationFlow) to override which states are set.
    """
    picking_state = flow_state_picking or EditLocationFlow.picking_inbounds
    manual_state = flow_state_manual or EditLocationFlow.waiting_inbounds

    try:
        async with XuiClient(base_url, api_token) as xui:
            inbounds = await xui.list_inbounds()
    except Exception as exc:
        log.warning("Could not fetch inbounds for location wizard: %s", exc)
        inbounds = []

    if inbounds:
        await state.update_data(picker_inbounds=inbounds)
        await state.set_state(picking_state)
        back_data = (
            f"{keyboards.CB_ADM_LOC_DETAIL_PREFIX}{loc_id}"
            if loc_id else keyboards.CB_ADM_HOME
        )
        kb = keyboards.admin_inbound_picker(
            inbounds,
            current_inbound_ids,
            back_data=back_data,
        )
        await message.answer(
            texts.EDIT_LOC_PROMPT_INBOUNDS_PICKER,
            reply_markup=kb,
            parse_mode=ParseMode.HTML,
        )
    else:
        # Fallback: manual text input
        await state.set_state(manual_state)
        current = ",".join(str(i) for i in current_inbound_ids) or "—"
        back_data = (
            f"{keyboards.CB_ADM_LOC_DETAIL_PREFIX}{loc_id}"
            if loc_id else keyboards.CB_ADM_HOME
        )
        await message.answer(
            texts.EDIT_LOC_INBOUNDS_FETCH_FAIL
            + f"\n\nفعلی: <code>{current}</code>",
            reply_markup=keyboards.admin_flow_cancel_inline(back_data=back_data),
            parse_mode=ParseMode.HTML,
        )
...
@router.callback_query(
    ...
    StateFilter(EditLocationFlow.picking_inbounds),
)
async def cb_inbound_toggle(
    callback: CallbackQuery,
    state: FSMContext,
    settings: Settings,
    db: Database,
...
async def cb_inbound_skip(
    callback: CallbackQuery,
    state: FSMContext,
    settings: Settings,
    db: Database,
) -> None:
    """Admin chose to enter inbound IDs manually instead of using the picker."""
    if not await guard_admin_callback(callback, settings, db, LOCATIONS):
        return
    if not isinstance(callback.message, Message):
        await callback.answer()
    ...
    data = await state.get_data()
    loc_id = int(data["loc_id"])
    current = ",".join(str(i) for i in data.get("inbound_ids", [])) or "—"
