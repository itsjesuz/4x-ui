...
func (r *Remote) DeleteClient(ctx context.Context, email string) error {
	...
	_, err := r.do(ctx, http.MethodPost, "panel/api/clients/del/"+url.PathEscape(email), nil)
	if err == nil {
		return nil
	}
	if strings.Contains(strings.ToLower(err.Error()), "not found") {
...
func (r *Remote) ResetInboundTraffic(ctx context.Context, ib *model.Inbound) error {
	...
	_, err = r.do(ctx, http.MethodPost, "panel/api/inbounds/"+strconv.Itoa(id)+"/resetTraffic", nil)
	return err
}
