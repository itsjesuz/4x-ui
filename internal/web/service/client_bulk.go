...
func (s *ClientService) BulkDelete(inboundSvc *InboundService, emails []string, keepTraffic bool) (BulkDeleteResult, bool, error) {
	...
	if len(successIds) > 0 {
		...
		// Clean up client records from remote nodes completely
		nodeToEmails := make(map[int][]string)
		for inboundId, ibEmails := range emailsByInbound {
			if inbound, getErr := inboundSvc.GetInbound(inboundId); getErr == nil && inbound.NodeID != nil {
				nodeToEmails[*inbound.NodeID] = append(nodeToEmails[*inbound.NodeID], ibEmails...)
			}
		}
		for nodeID := range nodeToEmails {
			if dErr := (&NodeService{}).MarkNodeDirty(nodeID); dErr != nil {
				logger.Warning("mark node dirty failed:", dErr)
			}
		}
	}
