...
import (
	...
	"github.com/itsjesuz/4x-ui/internal/xray"
...
func (s *ClientService) ListPaged(inboundSvc *InboundService, settingSvc *SettingService, params ClientPageParams) (*ClientPageResponse, error) {
	...
	subIDs := make([]string, 0, len(pageRows))
	...
	if len(subIDs) > 0 {
		if tgMap, err := queryTelegramInfoForSubIDs(subIDs); err == nil && tgMap != nil {
			for i, item := range items {
				if info, ok := tgMap[item.SubID]; ok {
					items[i].TelegramID = info.UserID
					items[i].TelegramUsername = info.Username
					items[i].TelegramFirstName = info.FirstName
					items[i].TelegramLastName = info.LastName
					if info.Nickname != "" {
						items[i].OrderNumber = info.Nickname
					} else if info.OrderID > 0 {
						items[i].OrderNumber = fmt.Sprintf("nf%d", info.OrderID)
					}
				}
			}
		}
...
func queryTelegramInfoForSubIDs(subIDs []string) (map[string]TelegramInfo, error) {
	if len(subIDs) == 0 {
		return nil, nil
	}
	dbPaths := []string{
		"/opt/netfly/netfly.db",
		"netfly.db",
		"../netfly.db",
		"C:\\Users\\Litebeet\\Desktop\\Netfly\\netfly.db",
	}
	...
	if db == nil {
		return nil, fmt.Errorf("could not open netfly.db")
	}
	...
	var rows []struct {
		...
		XuiSubID  string `gorm:"column:xui_sub_id"`
	}
	
	query := `
		SELECT u.user_id, u.username, u.first_name, u.last_name, o.id, o.nickname, o.xui_sub_id
		FROM orders o
		JOIN users u ON o.user_id = u.user_id
		WHERE o.xui_sub_id IN ?
	`
	err = db.Raw(query, subIDs).Scan(&rows).Error
	if err != nil {
		return nil, err
	}
	
	res := make(map[string]TelegramInfo)
	for _, r := range rows {
		if r.XuiSubID == "" {
			continue
		}
		res[r.XuiSubID] = TelegramInfo{
			UserID:    r.UserID,
			Username:  r.Username,
