...
func (s *ClientService) Delete(inboundSvc *InboundService, id int, keepTraffic bool) (bool, error) {
	...
	// Clean up client record from remote nodes completely
	if existing.Email != "" {
		nodeIDs := make(map[int]struct{})
		for _, ibId := range inboundIds {
			if inbound, getErr := inboundSvc.GetInbound(ibId); getErr == nil && inbound.NodeID != nil {
				nodeIDs[*inbound.NodeID] = struct{}{}
			}
		}
