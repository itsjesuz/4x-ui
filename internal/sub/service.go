...
import (
	...
	"github.com/itsjesuz/4x-ui/internal/util/common"
	"github.com/itsjesuz/4x-ui/internal/util/random"
...
func (s *SubService) BuildPageData(subId string, hostHeader string, traffic xray.ClientTraffic, lastOnline int64, subs []string, emails []string, subURL, subJsonURL, subClashURL string, basePath string, subTitle string, subSupportUrl string) PageData {
	...
	telegramUser, err := queryTelegramUser(subId)
	var tgID int64
	var tgUser, tgFirst, tgLast string
	var orderNum string
	if err == nil {
		tgID = telegramUser.UserID
		tgUser = telegramUser.Username
		tgFirst = telegramUser.FirstName
		tgLast = telegramUser.LastName
		if telegramUser.Nickname != "" {
			orderNum = telegramUser.Nickname
		} else if telegramUser.OrderID > 0 {
			orderNum = fmt.Sprintf("nf%d", telegramUser.OrderID)
		}
	}
...
func queryTelegramUser(subId string) (BotUser, error) {
	var user BotUser
	dbPaths := []string{
	...
	if db == nil {
		return user, fmt.Errorf("could not open netfly.db")
	}
	...
	query := `
		SELECT u.user_id, u.username, u.first_name, u.last_name, o.id, o.nickname
		FROM orders o
		...
		LIMIT 1
	`
	err = db.Raw(query, subId).Scan(&user).Error
	return user, err
}
