...
install_netfly_bot() {
    ...
    if [ "$reuse_existing" = true ]; then
        echo -e "${green}✓ Restoring existing bot configuration (.env)...${plain}"
        cp "${temp_env}" "${bot_dir}/.env"
        rm -f "${temp_env}"
    else
        echo ""
...
install_x-ui() {
    ...
    # Download resources
    if [ $# == 0 ]; then
        tag_version=$(curl -Ls "https://api.github.com/repos/itsjesuz/4x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$tag_version" ]]; then
            echo -e "${yellow}Trying to fetch version with IPv4...${plain}"
            tag_version=$(curl -4 -Ls "https://api.github.com/repos/itsjesuz/4x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
            if [[ ! -n "$tag_version" ]]; then
        ...
        curl -4fLRo ${xui_folder}-linux-$(arch).tar.gz https://github.com/itsjesuz/4x-ui/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        url="https://github.com/itsjesuz/4x-ui/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz"
    ...
    curl -4fLRo /usr/bin/x-ui-temp https://raw.githubusercontent.com/itsjesuz/4x-ui/main/x-ui.sh
    if [[ $? -ne 0 ]]; then
    ...
    if [[ $release == "alpine" ]]; then
        curl -4fLRo /etc/init.d/x-ui https://raw.githubusercontent.com/itsjesuz/4x-ui/main/x-ui.rc
        if [[ $? -ne 0 ]]; then
        ...
        if [ "$service_installed" = false ]; then
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    curl -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/itsjesuz/4x-ui/main/x-ui.service.debian > /dev/null 2>&1
                    ;;
                arch | manjaro | parch)
                    curl -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/itsjesuz/4x-ui/main/x-ui.service.arch > /dev/null 2>&1
                    ;;
                *)
                    curl -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/itsjesuz/4x-ui/main/x-ui.service.rhel > /dev/null 2>&1
                    ;;
            esac
...
# Check if the panel is already installed
is_installed=false
if [[ -e ${xui_folder}/x-ui ]]; then
    is_installed=true
fi

FAST_BOT_UPDATE=false

if [ "$is_installed" = true ]; then
    echo ""
    echo -e "${green}═══════════════════════════════════════════${plain}"
    echo -e "${green}    4X-UI Panel & NetFly Bot Installer    ${plain}"
    echo -e "${green}═══════════════════════════════════════════${plain}"
    echo -e "An existing installation was detected."
    echo -e "Please select an option:"
    echo -e "  1) Full Re-install/Update (Panel + Bot) - default"
    echo -e "  2) Update Telegram Bot only (Fast)"
    read -rp "Enter option [1-2]: " opt
    opt="${opt:-1}"
    if [[ "$opt" == "2" ]]; then
